 ...
 * 5) عبّي شيت "المستخدمات" بعمودين أساسيين فقط: "اسم المستخدم" (أي اسم فريد
 *    تختارينه - مو شرط إيميل حقيقي) و"كلمة المرور"، بالإضافة لعمود "الدور"
 *    اللي يحدد صلاحياتها. الوحدات والمراكز تُضاف كصفوف عادية بنفس الشيت
 *    (نفس أي موظفة)، بس بدور "مديرة مركز" مثلًا.
 *    ⚠️ مهم: كل صف لازم يكون له "اسم مستخدم" مختلف تمامًا عن باقي الصفوف -
 *    لو كررتي صف لإنشاء حساب جديد، لا تنسي تغيّرين عمود "اسم المستخدم" وإلا
 *    الحسابين بيشتركان بنفس التقرير بالضبط.
...
function setup() {
  ...
  const sheets = {
    'المستخدمات': ['اسم المستخدم', 'كلمة المرور', 'الدور', 'القسم', 'الوحدة'],
  ...
  if (usersSheet.getLastRow() === 1) {
    usersSheet.appendRow(['اسم تجريبي', '1234', 'موظفة', 'قسم البرامج القرآنية', 'وحدة تجريبية']);
  }
...
function login_(p) {
  ...
  const displayName = String(found['اسم المستخدم']).trim();
...
/* قائمة الدخول المنسدلة - بنفس فكرة نظام المقاصف: بدل ما تكتب الموظفة أو
   الوحدة اسمها يدويًا، تختارها من قائمة جاهزة معبّاة من شيت "المستخدمات"
   مباشرة (نفس الشيت لكل أنواع الحسابات - موظفة، مسؤولة وحدة، مديرة، مركز...).
   عمود واحد فقط يحدّد هوية الحساب (اسم المستخدم) - ما فيه عمود اسم منفصل
   ممكن ينسى تحديثه لو تكرر صف بالغلط، فما يصير تضارب بين حسابين. */
function getLoginOptions_(p) {
  const userRows = sheetToObjects_(USERS_SHEET_, 300);
  const users = userRows
    .map(function (r) {
      const username = String(r['اسم المستخدم'] || '').trim();
